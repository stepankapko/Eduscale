...
const Icon = ({ name, size = 16 }) => {
  const paths = {
    ...
    mic: <><rect x="9" y="3" width="6" height="12" rx="3" /><path d="M5 11C5 15 8 18 12 18M12 18C16 18 19 15 19 11M12 18V22M8 22H16" /></>,
    wave: <><path d="M3 12H5M21 12H19M7 8V16M11 5V19M15 7V17M19 9V15" /></>,
    quote: <><path d="M7 7H11V11C11 13 10 14 8 14M13 7H17V11C17 13 16 14 14 14" /></>,
    play: <><path d="M7 5L19 12L7 19Z" /></>
  };
  return (
...
<button className="logout-btn" onClick={onLogout} title="Odhlásit se">
    <Icon name="arrow" size={12}/> Odhlásit
  </button>;

const UserCard = ({ role, onLogout }) => {
  const r = ROLES[role];
  return (
    <div className="user-card">
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        ...
        <div className="user-role">{r.subtitle}</div>
      </div>
      {onLogout && <button className="icon-btn" onClick={onLogout} title="Odhlásit se"><Icon name="x" size={12}/></button>}
    </div>);

};

const Sidebar = ({ role, route, setRoute, counts, onLogout }) => {
  const nav = {
    coordinator: [
    ...
    { key: "e-interventions", label: "Intervence", icon: "clipboard", count: counts.interventions },
    { key: "e-people", label: "Účastníci", icon: "users", count: counts.participants },
    { key: "e-library", label: "Knihovna evaluací", icon: "library", count: counts.libraryDocs },
  ...
  return (
    <aside className="sidebar">
      <Logo />
      <div className="nav-section-label">{role === "coordinator" ? "Pracovní plocha" : "Evaluace"}</div>
      ...
      <UserCard role={role} onLogout={onLogout} />
    </aside>);
...
// === Form step indicator (used in onboarding + create form + add participant modal) ===
...
// === SchoolPicker: filter + pick (no free text) ===
  ...
  return (
    <div>
      <div className="filterbar" style={{ padding: 8, marginBottom: 10 }}>
        ...
        <input className="input" placeholder="filtrovat dle názvu / REDIZO" value={q} onChange={e => setQ(e.target.value)} style={{ flex: 1, border: "none", background: "transparent" }}/>
      </div>
...
Object.assign(window, { Icon, Logo, RoleSwitcher, LogoutButton, UserCard, Sidebar, TopBar, PageHead, Chip, Metric, PickList, EmptyState, FormStep, SchoolPicker, ROLES });